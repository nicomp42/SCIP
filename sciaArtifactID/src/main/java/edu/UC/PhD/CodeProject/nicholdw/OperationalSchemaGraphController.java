...
public class OperationalSchemaGraphController {
	...
	public void generateSchemaNodes(SchemaChangeImpactProject scip) {
		...
		try {
			...
			Neo4jUtils.ExecActionQuery("MERGE(m:Schema{name:'" + scip.getOperational().getOperationalSchemaName() + "',type:'" + "Operational" + "'})");
	...
	public void generateRelationNodes(SchemaChangeImpactProject scip) {
		try {
			Neo4jUtils.ExecActionQuery("LOAD CSV WITH HEADERS FROM \"" +
									   Neo4jUtils.filePrefix + Utils.formatPath(SchemaChangeImpactProject.operationalSubDirectory)  + "tables.csv\" AS line " +
	...
	public void generateAttributeNodes(SchemaChangeImpactProject scip) {
		try {
			Neo4jUtils.ExecActionQuery("LOAD CSV WITH HEADERS FROM \""
							+ Neo4jUtils.filePrefix + Utils.formatPath(SchemaChangeImpactProject.operationalSubDirectory) + "attributes.csv\" AS line "
	...
	public void generateReferentialIntegrityRelationships(SchemaChangeImpactProject scip) {
		try {
			Neo4jUtils.ExecActionQuery("LOAD CSV WITH HEADERS FROM \""
								+ Neo4jUtils.filePrefix + Utils.formatPath(SchemaChangeImpactProject.operationalSubDirectory) + "foreignKeys.csv\" AS line "
	...
	public void generateSchemaRelationRelationships(SchemaChangeImpactProject scip) {
		try {
			Neo4jUtils.ExecActionQuery("LOAD CSV WITH HEADERS FROM \""
								+ Neo4jUtils.filePrefix + Utils.formatPath(SchemaChangeImpactProject.operationalSubDirectory) + "tables.csv\" AS line MATCH (src:Schema {name: line.schema_name}) "
	...
	public void deleteSchema() {
		...
		try {
			...
			Neo4jUtils.ExecActionQuery("MATCH (n) DETACH DELETE n");
