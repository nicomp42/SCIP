...
//	private static final String provenanceRealationName = "Impacts";
	...
	public static boolean executeCypherQueries(String filePath) {
		...
		try {
			...
			QueryGraphVersion01.executeCypherQueries(Config.getConfig().getNeo4jSuffix(), AttributeProvenanceForNe04j.cypherQueriesStep1, Utils.formatPath(filePath));	// Folder defaults to the import folder in the Neo4j project structure

			String fileName = Utils.formatPath(Utils.cleanPath(filePath)) + Config.getConfig().getNeo4jSuffix() + attributeProvenanceFileSuffix + Config.getConfig().getCSVFileExtension();
			ProcessCSVFileByLine(fileName);

			// Run the queries that build the relationships
			QueryGraphVersion01.executeCypherQueries(Config.getConfig().getNeo4jSuffix(), AttributeProvenanceForNe04j.cypherQueriesStep2, Utils.formatPath(filePath));	// Folder defaults to the import folder in the Neo4j project structure
