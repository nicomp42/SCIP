...
public class DimensionLookupStepParser {
	...
	public static DimLookupUpdateStep parseXMLForDimensionLookupStep(
		...
		stepObject = new DimLookupUpdateStep(transname, stepName,
				"Dimension Lookup", dbname, tableName, fieldnames, isUpdateStep);

		return stepObject;
	}

	private static boolean getStepUpdateStatus(Document doc, XPath xpath,
			String stepname) {
		boolean updateStatus = false;
		try {
			XPathExpression expr = xpath.compile("/transformation/step[name=\'"
					+ stepname + "\']/update/text()");
			String updateStatus_str = (String) expr.evaluate(doc,
					XPathConstants.STRING);
			updateStatus = (updateStatus_str.equals("Y") ? true : false);
			System.out.println("It is an update step:" + updateStatus);
	...
	private static List<String> getDatabaseFieldNames(Document doc,
		...
		try {
			...
			XPathExpression expr_lookupkey_field = xpath
					.compile("/transformation/step[name=\'" + stepname
							+ "\']/fields/key/lookup/text()");
			XPathExpression expr_daterange_start = xpath
					.compile("/transformation/step[name=\'" + stepname
							+ "\']/fields/date/from/text()");
			XPathExpression expr_daterange_end = xpath
					.compile("/transformation/step[name=\'" + stepname
							+ "\']/fields/date/to/text()");
			XPathExpression expr_version_field = xpath
					.compile("/transformation/step[name=\'" + stepname
							+ "\']/fields/return/version/text()");
			XPathExpression expr_surrogate_key = xpath
					.compile("/transformation/step[name=\'" + stepname
							+ "\']/fields/return/name/text()");
			XPathExpression expr_updateOrInsertFields = xpath
					.compile("/transformation/step[name=\'" + stepname
							+ "\']/fields/field/lookup/text()");
			// evaluate expression result on XML document
			NodeList nodes = (NodeList) expr_updateOrInsertFields.evaluate(doc,
					XPathConstants.NODESET);
			for (int i = 0; i < nodes.getLength(); i++)
				list.add(nodes.item(i).getNodeValue());
			String lookupkey_field = (String) expr_lookupkey_field.evaluate(
					doc, XPathConstants.STRING);
			String daterange_start_field = (String) expr_daterange_start
					.evaluate(doc, XPathConstants.STRING);
			String daterange_end_field = (String) expr_daterange_end.evaluate(
					doc, XPathConstants.STRING);
			String version_field = (String) expr_version_field.evaluate(doc,
					XPathConstants.STRING);
			String surrogatekey_field = (String) expr_surrogate_key.evaluate(
					doc, XPathConstants.STRING);
			// for (int i = 0; i < nodes.getLength(); i++)
			// list.add(nodes.item(i).getNodeValue());
			if (lookupkey_field != null && !lookupkey_field.isEmpty())
				list.add(lookupkey_field);
			if (daterange_start_field != null
					&& !daterange_start_field.isEmpty())
				list.add(daterange_start_field);
			if (daterange_end_field != null && !daterange_end_field.isEmpty())
				list.add(daterange_end_field);
			if (version_field != null && !version_field.isEmpty())
				list.add(version_field);
